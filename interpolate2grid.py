...
# ----------  Functions ----------
def processinputfiles( nc_dataname='l3m_data', nc_latname='G3fakeDim0', nc_lonname='G3fakeDim1', 
                                        csv_lonname='LONG', csv_latname='LAT',  debug=False):
    """ Extract and standardise input data to array form from given NetCDF and CSV files """
    # --- Get Array file (HDF) and file of lats and lons from 1st and 2nd arguments respectively
    arr_file = sys.argv[1]
...
def interpolate_locs2grid( data, lon, lat, locs  ):
    """ Interpolate over given array (data) for locations (locs) at Lon. and Lat. , returning values """
...
# -----------------------------------------------------------------------------
# ----------  Driver ----------
""" Setup output here - change variables names to columns headers within given files
...
debug=True
nc_dataname='l3m_data'
nc_latname='G3fakeDim0'
nc_lonname='G3fakeDim1' 
csv_lonname='LONG'
csv_latname='LAT'

# ---------- 
# Get files and process to arrays, then mask for values below 0
data, lon, lat, locs  = processinputfiles(nc_dataname=nc_dataname,  nc_latname=nc_latname, 
                    nc_lonname=nc_lonname, csv_lonname=csv_lonname, csv_latname=csv_latname)
print [ [np.ma.min(i), np.ma.max(i), np.ma.mean(i), np.ma.std(i) ] for i in [data] ]
...
df = pd.DataFrame( {'Interp. Values': vals, 'Lon.':locs[0,:], 'Lat.':locs[1,:], 'flag': flags } )
...
#    vmin, vmax = data.min(), data.max()
    ...
    cmap=plt.cm.Greens
    s_z = [ cmap(  (float(i) - vmin) / ( np.array([vmin,vmax]) ).ptp() ) for i in vals ]
    print vmin, vmax
    plt.pcolor( lon, lat, data.T, cmap=plt.cm.Blues, vmin=vmin, vmax=vmax )
    plt.scatter( locs[0,:], locs[1,:] , cmap=plt.cm.Greens, c=vals,vmin=vmin, marker='x', vmax=vmax  )    
    plt.show()
